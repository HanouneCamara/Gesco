<?php
    ...
    if ($_SESSION['poste'] == 'admin'){
        $name = $_POST['name'];
        $req = "SELECT * FROM users WHERE username LIKE'%$name%' AND poste NOT LIKE ('admin') AND departement = '$user_departement' AND presence = 1";
        $rs = $conn->query($req);
        $row_cnt = mysqli_num_rows($rs);
    }else if($_SESSION['poste'] == 'directeur'){
        $name = $_POST['name'];
        $req = "SELECT * FROM users WHERE username LIKE'%$name%' AND presence = 1";
        $rs = $conn->query($req);
        $row_cnt = mysqli_num_rows($rs);
    }
