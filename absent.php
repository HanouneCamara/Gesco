<?php
    ...
    if ($_SESSION['poste'] == 'admin'){
        $req = "SELECT * FROM users WHERE poste NOT LIKE ('admin') AND poste NOT LIKE ('directeur') AND departement = '$user_departement' AND presence = 0";
        $rs = $conn->query($req);
    }else if($_SESSION['poste'] == 'directeur'){

        $req = "SELECT * FROM users WHERE poste NOT LIKE ('admin') AND poste NOT LIKE ('directeur') AND presence = 0";
        $rs = $conn->query($req);
    }
?>
